...
if not api_key:
    ...
    st.info("Go to: Settings → Secrets → paste: GEMINI_API_KEY = 'your-key-here'")
    st.stop()
...
EVAL_PROMPT = """You are an expert LLM prompt-response quality evaluator.
...
st.markdown('<p style="color:#666;font-family:\'IBM Plex Mono\',monospace;font-size:13px;">Powered by Gemini 1.5 Flash · Score · Evaluate · Compare · Improve</p>', unsafe_allow_html=True)
st.markdown("---")
...
with tab1:
    ...
    st.caption("Paste any prompt and an AI-generated response. Gemini will score it across 5 quality parameters.")
    ...
    if st.button("Evaluate →", key="btn_single"):
        if not prompt_input.strip() or not response_input.strip():
            ...
            with st.spinner("Gemini is evaluating across 5 parameters..."):
                try:
                    ...
                    st.error("Could not parse Gemini's response as JSON. Please try again.")
...
with tab2:
    ...
    st.caption("Submit the same prompt with two different AI responses. Gemini scores both and declares a winner.")
    ...
    if st.button("Compare Both →", key="btn_compare"):
        if not compare_prompt.strip() or not response_a.strip() or not response_b.strip():
            ...
            with st.spinner("Gemini is scoring both responses..."):
                try:
